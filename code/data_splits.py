...
def get_train_val_data(all_data, train_ind, val_ind, adj, seed):
    ...
    # print('train_data_pos:', train_data_pos.shape, val_data_pos.shape)
...
def neg_data_generate(adj_data_all, train_data_fix, val_data_fix, neg_num_test, seed):
    ...
    train_neg_all = np.vstack(
        (np.vstack((np.vstack((np.vstack((train_neg_1_arr, train_neg_2_arr)), train_neg_3_arr)), train_neg_4_arr)),
         train_data_fix))
    np.random.shuffle(train_neg_all) # 打乱训练集顺序
...
def generate_dataset():
    ...
    adj_data = np.loadtxt('./MDS_data/m_r_d_pos_pairs.txt')
    # adj_data = np.loadtxt('./MDS_data/m_r_d_pos_pairs.txt')
    print("adi_data",adj_data.shape)
    np.random.shuffle(adj_data)

    cv_data = adj_data[int(0.1 * len(adj_data)):, :]
    indep_data = adj_data[:int(0.1 * len(adj_data)), :]
    print("cv_data/indep_data",cv_data.shape,indep_data.shape)

    fold_num = 0
    kf = KFold(n_splits=5, shuffle=True, random_state=args.seed)
    for train_index, val_index in kf.split(cv_data):
        # print(cv_data[train_index], val_index.shape)
        fold_num += 1
        ...
        np.savetxt('./MDS_data/CV_data/CV_' + str(fold_num) + '/train_data_pos.csv', train_data_pos, delimiter=",",
                   fmt='%d')
        np.savetxt('./MDS_data/CV_data/CV_' + str(fold_num) + '/train_data_neg.csv', train_data_neg, delimiter=',',
                   fmt='%d')
        np.savetxt('./MDS_data/CV_data/CV_' + str(fold_num) + '/val_data_pos.csv', val_data_pos, delimiter=',', fmt='%d')
        np.savetxt('./MDS_data/CV_data/CV_' + str(fold_num) + '/val_data_neg.csv', val_data_neg, delimiter=',', fmt='%d')

        print('CV_', train_data_pos.shape, train_data_neg.shape, val_data_pos.shape, val_data_neg.shape)


    train_data_pos, train_data_neg, test_data_pos, test_data_neg = get_indep_data(adj_data, cv_data, indep_data,
                                                                                  args.seed)
    np.savetxt('./MDS_data/indepent_data/train_data_pos.csv', train_data_pos, delimiter=",", fmt='%d')
    np.savetxt('./MDS_data/indepent_data/train_data_neg.csv', train_data_neg, delimiter=",", fmt='%d')
    np.savetxt('./MDS_data/indepent_data/test_data_pos.csv', test_data_pos, delimiter=",", fmt='%d')
    np.savetxt('./MDS_data/indepent_data/test_data_neg.csv', test_data_neg, delimiter=",", fmt='%d')
    print('indep_', train_data_pos.shape, len(train_data_neg), test_data_pos.shape, test_data_neg.shape)
